...
export default function ApplicationDetail() {
  ...
  return (
    <div>
      ...
      {/* 12-hour edit lock banner */}
      {(app.status === 'pending' || app.status === 'submitted') && (
        <div className={`mb-4 rounded-xl px-4 py-3 text-sm ${canEdit ? 'bg-[#c6a15b]/15 text-[#0b1c33]' : 'bg-[#0b1c33]/8 text-[#0b1c33]/60'}`}>
          {canEdit ? (
            <>
              ✏️ <strong>Edit window open</strong> — {editTimeLabel}. You can edit personal details and replace documents.
            </>
          ) : (
            <>🔒 <strong>Editing locked</strong> — the 12-hour edit window has closed.</>
          )}
        </div>
      )}
